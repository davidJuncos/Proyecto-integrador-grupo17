...
class SistemaUsuarios:
    ...
    # @staticmethod
    # def agregar_usuario(usuario):
    #     """Agrega un nuevo usuario utilizando búsqueda binaria para ordenarlo por DNI."""
    #     usuarios = SistemaUsuarios.cargar_usuarios()

    #     # Encontrar la posición correcta mediante búsqueda binaria
    #     posicion = SistemaUsuarios.buscar_posicion_insercion(usuarios, usuario.dni)

    #     # Insertar el usuario en la posición encontrada
    #     usuarios.insert(posicion, usuario)

    #     # Guardar la lista de usuarios ordenada
    #     SistemaUsuarios.guardar_usuarios(usuarios)

    #     print(f"Usuario {usuario.username} agregado en la posición {posicion} y ordenado por DNI.")

    # @staticmethod
    # def buscar_posicion_insercion(usuarios, dni):
    #     """Usa búsqueda binaria para encontrar la posición donde insertar el nuevo usuario."""
    ...
    #         f"buscandoUsuarioPorDNI-{datetime.now().strftime('%Y-%m-%d')}.txt"
    #     )
    ...
    #         fecha_actual = datetime.now()
    #         # with open(SistemaUsuarios.FILE_NAME_LOGS_BUSQUEDA_BINARIA_DNI, 'a') as log_file:
    #         #     log_file.write(f"[{fecha_actual}] Intento fallido de acceso - DNI: {dni_buscar_int}\n")
    #         # print(f"Se registró el intento fallido de {dni_buscar_int} en 'logi.txt'.")
            
    #         with open(SistemaUsuarios.FILE_NAME_LOGS_BUSQUEDA_BINARIA_DNI, 'a') as log:
    #             log.write(f"Búsqueda Binaria por DNI: buscando el DNI {dni_buscar} en {total_usuarios} usuarios.\n")
    ...
    #             log.write(f"No se encontró el DNI en {intentos} intentos.\n")
    #     except IOError as e:
    ...
    #     for usuario in usuarios:
    #         intentos += 1
    #         print(f"Intento {intentos}: {username_buscar} {'es igual' if usuario.username == username_buscar else 'es distinto'} a {usuario.username}")

    #         if usuario.username == username_buscar:
    #             print(f"Se encontró en {intentos} intentos.")
    #             return usuario
